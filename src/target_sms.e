...
export procedure output_sms(sequence args)
    atom factor
    integer f, machineSpeed, tableSize, cbSize, patSize, songSize
    ...
    if args[1] then
        ...
        LOGIF(verbose, "Size of effect tables: %d bytes\n", tableSize)
        ...
        LOGIF(verbose, "Size of callback table: %d bytes\n", cbSize)
        ...
        puts(outFile, CRLF & "xpmp_pattern_tbl:" & CRLF)
        for n = 1 to length(patterns[2]) do
            printf(outFile, ".dw xpmp_pattern%d" & CRLF, n)
            patSize += 2
        end for
        puts(outFile, CRLF)

        LOGIF(verbose, "Size of patterns table: %d bytes\n", patSize)
        ...
        LOGIF(verbose, "Total size of song(s): %d bytes\n", songSize + patSize + tableSize + cbSize)
